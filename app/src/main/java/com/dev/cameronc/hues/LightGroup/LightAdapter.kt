...
import com.dev.cameronc.hues.Home.LightGroupAdapter
 ...
 */
class LightAdapter(val lights: List<PHLight>, val lightInteractionListener: LightInteractionListener) : RecyclerView.Adapter<LightGroupAdapter.LightGroupVH>()
{
    var sliderListener: LightSliderListener? = null

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): LightGroupAdapter.LightGroupVH
    {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.light_group_item, parent, false)
        return LightGroupAdapter.LightGroupVH(view)
    }

    override fun onBindViewHolder(holder: LightGroupAdapter.LightGroupVH, position: Int)
    {
